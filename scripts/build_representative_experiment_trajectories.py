...
def _target_image_path(bundle_root: Path, target_id: str) -> Path:
    ...
    return targets_dir / target_map[target_id]
...
def _load_run_payload(run: CuratedRun) -> dict[str, object]:
    ...
    target_path = _target_image_path(bundle_root, run_row["target_id"])
    ...
    mid_round = 4 if 4 in by_round else sorted(by_round)[min(len(by_round) - 1, 2)]
    return {
        "row_label": run.row_label,
        "target_path": target_path,
        "baseline": {"label": "Baseline", "path": _repo_path(baseline_row["image_path"]), "clip": float(baseline_row["clip_score"])},
        "round_1": {"label": "Best by R1", "path": _repo_path(by_round[1]["image_path"]), "clip": float(by_round[1]["clip_score"])},
        "round_mid": {
            "label": f"Best by R{mid_round}",
            "path": _repo_path(by_round[mid_round]["image_path"]),
            "clip": float(by_round[mid_round]["clip_score"]),
        },
        "final": {"label": "Final best", "path": _repo_path(by_round[final_round]["image_path"]), "clip": float(by_round[final_round]["clip_score"])},
    }


def build_figure(output_path: Path) -> None:
    payloads = [_load_run_payload(run) for run in RUNS]
    ...
    draw.text((margin, 16), "Representative archived refinement trajectories", fill="#2b221a", font=title_font)
    draw.text(
        (margin, 52),
        "Experiment-derived trajectories with clear best-so-far improvement under the hidden-target oracle protocol.",
        fill="#6e5a47",
        font=small_font,
    )
    ...
    for row_index, payload in enumerate(payloads):
        ...
        for col_offset, key in enumerate(["baseline", "round_1", "round_mid", "final"], start=1):
            ...
            draw.rounded_rectangle((x, y, x + tile_w, y + tile_h), radius=14, outline="#d6c7b6", width=2)
            draw.text((x + 4, y + tile_h + 8), f"{panel['label']} | CLIP {panel['clip']:.3f}", fill="#4e4034", font=small_font)
...
def main() -> int:
    output_path = FIGURE_ROOT / "figure_4_case_study_progression.png"
