...
stagnation_trust_radius_scale: 1.4
anchor_strength: 0.2
guidance_scale: 8.0
...
"""
    ...
    assert config.sampler == "diversity_shell"
    assert config.updater == "score_weighted_preference"
    assert config.feedback_mode.value == "top_k"
    assert config.guidance_scale == 8.0
    assert config.num_inference_steps == 25
    assert config.stagnation_patience == 2
    assert config.stagnation_trust_radius_scale == 1.4
